...
import {
  ...
  Printer, PencilRuler, ScanLine, Wrench, MessageSquare, Download, User, Filter,
} from "lucide-react";
...
function SkeletonCard() {
  return (
    <div className="rounded-2xl border border-border/20 bg-card/30 p-5 animate-pulse">
...
type FilterTab = "all" | "active" | "done" | "new" | "working" | "shipped";
const FILTER_TABS: { key: FilterTab; label: string; icon?: React.ElementType }[] = [
  ...
  { key: "working", label: "В работе", icon: Printer },
  { key: "shipped", label: "Отправлен", icon: Truck },
  { key: "done", label: "Завершённые", icon: CheckCircle },
];

function filterOrders(orders: Order[], tab: FilterTab): Order[] {
  if (tab === "all") return orders;
  if (tab === "active") return orders.filter(o => !["completed","rejected","confirmed","cancelled"].includes(o.status));
  if (tab === "done") return orders.filter(o => ["completed","rejected","confirmed"].includes(o.status));
  if (tab === "new") return orders.filter(o => ["new","calculating"].includes(o.status));
  if (tab === "working") return orders.filter(o => ["accepted","working","printing","postprocess"].includes(o.status));
  if (tab === "shipped") return orders.filter(o => ["ready","shipped"].includes(o.status));
  return orders;
}
      ...
      <div className="container mx-auto px-4 py-10 max-w-3xl relative z-10">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap border transition-all ${
                    filterTab === tab.key
                      ...
                      : "bg-card/20 border-border/30 text-white/40 hover:text-white/70 hover:border-border/50"
                  }`}
                >
                  {tab.icon && <tab.icon className="w-3 h-3" />}
                  {tab.label}
                  <span className={`ml-0.5 text-[10px] px-1.5 py-0.5 rounded-full font-bold ${
                    filterTab === tab.key ? "bg-primary/30 text-primary" : "bg-white/5 text-white/30"
                  }`}>
          ...
          {isLoading ? (
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              className="text-center py-14 rounded-2xl border border-dashed border-border/30 bg-card/10">
              <Filter className="w-8 h-8 text-white/20 mx-auto mb-3" />
              <p className="text-white/30 text-sm">Заказов в этом фильтре нет</p>
            </motion.div>
          ) : (
            <AnimatePresence mode="wait">
              <motion.div key={filterTab} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                className="space-y-3">
